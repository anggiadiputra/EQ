<script>
  ...
  function searchPermissions() {
    router.get('/admin/permissions', { search, category: selectedCategory }, {
      preserveState: true,
      replace: true
    });
  }
  ...
  function deletePermission() {
    if (selectedPermission) {
      router.delete(`/admin/permissions/${selectedPermission.id}`, {
        onSuccess: () => {
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        ...
        <p class="text-gray-600">Kelola permission yang tersedia di sistem</p>
      </div>
        ...
        <div class="flex-1">
          <div class="relative">
            <input
              type="text"
              bind:value={search}
              on:keydown={(e) => e.key === 'Enter' && searchPermissions()}
              ...
              <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                <div>
                  ...
                  <div class="text-sm text-gray-500">
                    Created: {new Date(permission.created_at).toLocaleDateString('id-ID')}
                  </div>
                </div>
    ...
    <!-- Permissions Table (for search results) -->
    {#if search || selectedCategory}
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {categories[permission.name.split('.')[0]] || permission.name.split('.')[0]}
                    </span>
                  </td>
        ...
        {#if permissions.data.length === 0}
          <div class="text-center py-8">
            <p class="text-gray-500">Tidak ada permission ditemukan.</p>
          </div>
        {/if}
      </div>

      <!-- Pagination -->
      {#if permissions.links && permissions.links.length > 3}
    ...
    {#if Object.keys(groupedPermissions).length === 0 && permissions.data.length === 0}
      <div class="text-center py-8">
        <p class="text-gray-500">Tidak ada permission ditemukan.</p>
      </div>
    {/if}
